...
class MAPAL(QueryStrategy):
    ...
    def compute_scores(self):
        ...
        print('is_unlabeled type : ',type(is_unlabeled))
        print('is_unlabeled shape : ',is_unlabeled.shape)
        unlabeled_indices = self.data_set_.get_unlabeled_indices()
        print('unlabeled_indices shape : ',unlabeled_indices.shape)
        n_labels_per_annotator = self.data_set_.len_labeled(per_annotator=True)
        print('n_labels_per_annotator : ',n_labels_per_annotator)
        scores = np.full((len(self.data_set_), self.data_set_.n_annotators_), np.nan)
        print('scores shape : (filled with nan )',scores.shape)
        

        # fit beta annotators model
        self.bam_.fit(self.data_set_.X_, self.data_set_.y_)
        print('fit beta annotators model')
        print(self.data_set_.X_.shape)
        print(self.data_set_.y_.shape)

        # estimate mean annotation performances
        print('estimate mean annotation performance')
        A_mean = self.bam_.predict_proba(self.data_set_.X_)[:, :, 0].T
        print('self bam prediction shape : ',self.bam_.predict_proba(self.data_set_.X_).shape)
        print(self.bam_.predict_proba(self.data_set_.X_)[:,0,:])
        ...
        #print('A_rand shape : ',A_rand.shape)
        #print('A_rand : ',A_rand)
        ...
        #print('annotator indices : ',annotator_indices.shape)
        #print('annotator indices : ',annotator_indices)
